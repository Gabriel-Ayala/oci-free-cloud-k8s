...
resource "oci_identity_user" "this" {
  ...
  name           = var.user_name
  description    = "Dedicated user for Mimir metrics storage in OCI Object Storage"
  email          = var.user_email
}

resource "oci_identity_customer_secret_key" "this" {
  display_name = "mimir-object-storage"
  user_id      = oci_identity_user.this.id
}

resource "oci_identity_policy" "this" {
  compartment_id = var.compartment_id
  name           = "MimirObjectStorage"
  description    = "Allow the Mimir storage user to manage objects in its bucket"
...
resource "oci_vault_secret" "access_key" {
  ...
  secret_name    = "mimir-s3-access-key"
...
resource "oci_vault_secret" "secret_key" {
  compartment_id = var.compartment_id
  key_id         = var.vault_key_id
  secret_name    = "mimir-s3-secret-key"
  vault_id       = var.vault_id
...
resource "oci_vault_secret" "endpoint" {
  compartment_id = var.compartment_id
  key_id         = var.vault_key_id
  secret_name    = "mimir-s3-endpoint"
  vault_id       = var.vault_id

  secret_content {
    content_type = "BASE64"
    content      = base64encode("${data.oci_objectstorage_namespace.this.namespace}.compat.objectstorage.${var.region}.oci.customer-oci.com")
  }
}

resource "oci_vault_secret" "bucket" {
  compartment_id = var.compartment_id
  key_id         = var.vault_key_id
  secret_name    = "mimir-s3-bucket"
  vault_id       = var.vault_id

  secret_content {
    content_type = "BASE64"
    content      = base64encode(var.bucket_name)
  }
}
