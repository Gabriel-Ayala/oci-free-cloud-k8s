...
through the tools Contour Gateway at `https://grafana-inova.amedsaude.com.br`.
...
at `https://keycloak-inova.amedsaude.com.br` with edge TLS. Custom realms, clients,
...
- an HTTP and HTTPS listener for `*.amedsaude.com.br`.
...
`storage-production.amedsaude.com.br`. The tools DNS record is outside the minimal
...
ExternalDNS is restricted to the configured AMED zones and manages A records
